        ...
        <div class="card shadow mb-4">
            <div >
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    <div>
                        <h2 class="m-0 font-weight-bold">Stock</h2>
                    </div>
                    <div>
                        <a href="{{ route("Stock.create") }}" id="ajouter" class="btn"><i class="fa fa-plus"></i></a>
                    </div>
                </div>
                @if(count($data) > 0)
                            ...
                            <tbody>
                                @foreach ($data as $item)
                                    <tr class="text-center searchable-element">
                                        ...
                                        <td>{{ $item->garantie }}</td>
                                        <td>{{ $item->qt }}</td>
                                        <td>{{ $item->sous_categories->name}}</td>
                                        <td>{{ $item->achat->ref }}</td>
                                        <td>
                                            <a href="{{ route('Stock.edit', $item->id) }}" class="btn text-warning"><i class='fas fa-pencil-alt'></i></a>
                                            <a href="#" class="btn delete-btn {{$item->produit->count()>0 ? ' text-secondary disabled' : "text-danger"}}" data-toggle="modal" data-target="#confirmDeleteModal" data-id="{{ $item->id }}"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    <script>
        $(document).ready(function() {
           ...
           $('#confirmDeleteBtn').on('click', function() {
               ...
               $.ajax({
                   url: '{{ route("Stock.destroy", "") }}/' + deleteId,
                   type: 'get',

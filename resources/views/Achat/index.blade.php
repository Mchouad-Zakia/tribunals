        ...
        <div class="card shadow mb-4">
            <div >
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    <div>
                        <h2 class="m-0 font-weight-bold">liste d'Achat</h2>
                    </div>
                    <div>
                        <a href="{{ route('Achat.create') }}" id="ajouter" class="btn "><i class="fa fa-plus"></i></a>
                    </div>
                </div>
                            ...
                            <tbody>
                                @foreach ($data as $item)
                                    <tr class="text-center searchable-element">
                                        <td>{{ $item->ref }}</td>
                                        <td>{{ $item->qt }}</td>
                                        <td>{{ $item->fournisseur->nom_fournisseur }}</td>
                                        <td>{{ $item->date }}</td>
                                        <td>{{ $item->description }}</td>
                                        <td>
                                            <a href="{{ route('Achat.edit', $item->id) }}" class="btn text-warning"><i class='fas fa-pencil-alt'></i></a>
                                            <a href="#" class="btn delete-btn {{$item->stock->count()>0 ? ' text-secondary disabled' : "text-danger"}}" data-toggle="modal" data-target="#confirmDeleteModal" data-id="{{ $item->id }}"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    <script>
        $(document).ready(function() {
           ...
           $('#confirmDeleteBtn').on('click', function() {
               ...
               $.ajax({
                   url: '{{ route("Achat.destroy", "") }}/' + deleteId,
                   type: 'get',
